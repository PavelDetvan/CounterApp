...
/**
 * Edit Counter Modal
 * Form to edit an existing counter's properties
 */
...
export default function EditCounterModal({ isOpen, onClose, counter }: EditCounterModalProps) {
  ...
  const { updateCounter } = useCounterStore();

  // Populate form with counter data when counter changes
  useEffect(() => {
    if (counter) {
      setName(counter.name);
      setDescription(counter.description || '');
      setColor(counter.color || '#3B82F6');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-900">Edit Counter</h2>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          {/* Counter Stats (Read-only) */}
          <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
            <h3 className="text-sm font-medium text-gray-700 mb-2">Counter Stats</h3>
